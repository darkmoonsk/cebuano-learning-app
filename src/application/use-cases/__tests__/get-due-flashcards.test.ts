...
describe("GetDueFlashcardsUseCase", () => {
  it("supplements missing flashcards with additional due items", async () => {
    const reviewStates = [createReviewState("card-1"), createReviewState("card-2")];
    ...
    const findByIdSpy = vi.fn().mockImplementation(async (id: string) =>
    ...
    const useCase = new GetDueFlashcardsUseCase(reviewRepository, flashcardRepository);
  ...
  it("tops up results to the default limit when there are fewer scheduled reviews", async () => {
    ...
    const findByIdSpy = vi.fn().mockImplementation(async (id: string) =>
      flashcards.find((card) => card.id === id) ?? null,
    );
